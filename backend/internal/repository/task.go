...
var allFieldsExceptId = []string{
	"title", "text", "completed", "counter_value", "counter_max_value", "counter_scale", "counter_exist", "user_id",
}
...
func (t *taskQuery) GetTasks(ctx context.Context, user string) ([]entities.Task, error) {
...
func (t *taskQuery) GetTask(ctx context.Context, id int64, user string) (*entities.Task, error) {
	task := entities.Task{}

	q, args, err := t.qb.Select(allFields...).
		From(taskTable).
		Where(sq.Eq{"id": id, "user_id": user}).
		ToSql()
	if err != nil {
...
func (t *taskQuery) CreateTask(ctx context.Context, task entities.Task) (*int64, error) {
	q, args, err := t.qb.Insert(taskTable).
		...
		Values(task.Title, task.Text, task.Completed, task.CounterValue,
			task.CounterMaxValue, task.CounterScale, task.CounterExist, task.UserId).
		Suffix("returning id").
		ToSql()
	if err != nil {
...
func (t *taskQuery) UpdateTask(ctx context.Context, task entities.Task, user string) (*entities.Task, error) {
	q, args, err := t.qb.Update(taskTable).
		...
		Where(sq.Eq{"id": task.ID, "user_id": user}).
...
func (t *taskQuery) DeleteTask(ctx context.Context, id int64, user string) error {
